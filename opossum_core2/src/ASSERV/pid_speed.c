...
void pid_vitesse_reset(void)
{
    ...
    pid_speed.err4.err = 0;
    pid_speed.err4.err_int = 0;
    pid_speed.err4.err_der = 0;
}
...
ESC_Command pid_speed_processing(PID_speed *pid, float err1, float err2, float err3, float err4)
{
    ...
    if (Pid_Speed_En)
    {
        ...
        pid->err4.err_der = err4 - pid->err4.err;
        ...
        if (pid->coef.ki != 0)
        {
            pid->err4.err_int += err4;
            float max_int = (MOTOR_POWER_MAX - (pid->err4.err * pid->coef.kp)) / pid->coef.ki;
            float min_int = (MOTOR_POWER_MIN - (pid->err4.err * pid->coef.kp)) / pid->coef.ki;
            pid->err4.err_int = limit_float(pid->err4.err_int, min_int, max_int);
        }

        commande.command1 = pid->coef.kp * pid->err1.err + pid->coef.ki * pid->err1.err_int +
                            pid->coef.kd * pid->err1.err_der;
        commande.command2 = pid->coef.kp * pid->err2.err + pid->coef.ki * pid->err2.err_int +
                            pid->coef.kd * pid->err2.err_der;
        commande.command3 = pid->coef.kp * pid->err3.err + pid->coef.ki * pid->err3.err_int +
                            pid->coef.kd * pid->err3.err_der;
        commande.command4 = pid->coef.kp * pid->err4.err + pid->coef.ki * pid->err4.err_int +
                            pid->coef.kd * pid->err4.err_der;
        return commande;
    }
    else
    {
