/**
 * @file BNO085.c
 * @brief Driver BNO085 (SHTP/SH-2 sur SPI) pour Zynq7000 — GPIO PS partagé
 *
 ...
 *
 * Les broches CS/RST/INT sont pilotées directement sur l'instance GPIO PS
 * partagée (dev->gpio_ctx->instance), et non via le cycle générique
 * PS_GPIO_Update : le séquencement CS doit se faire à l'intérieur même
 * d'une transaction SPI, ce que la mise à jour périodique ne permettrait pas.
 */
...
/* ═══════════════════════════════════════════════════════════════════════════
 * Macros GPIO — accès direct à l'instance PS GPIO partagée
 * ═══════════════════════════════════════════════════════════════════════════ */

#define CS_LOW(dev)    XGpioPs_WritePin(&(dev)->gpio_ctx->instance, (dev)->pin_cs, 0U)
#define CS_HIGH(dev)   XGpioPs_WritePin(&(dev)->gpio_ctx->instance, (dev)->pin_cs, 1U)
#define RST_LOW(dev)   XGpioPs_WritePin(&(dev)->gpio_ctx->instance, (dev)->pin_rst, 0U)
#define RST_HIGH(dev)  XGpioPs_WritePin(&(dev)->gpio_ctx->instance, (dev)->pin_rst, 1U)

/* INT est actif bas : retourne 0 quand le BNO085 a des données */
#define INT_READ(dev)  XGpioPs_ReadPin(&(dev)->gpio_ctx->instance, (dev)->pin_int)
...
static int spi_init(BNO085_Dev *dev)
{
     ...
     * car on gère CS manuellement via l'instance GPIO PS partagée.
     */
